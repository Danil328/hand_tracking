...
from .tools import nms, py_cpu_softnms
...
class HandTracker:
    ...
    def __init__(self, palm_model, joint_model, anchors_path,
        ...
        self.interp_palm = tf.contrib.lite.Interpreter(palm_model)
        self.interp_palm.allocate_tensors()
        self.interp_joint = tf.contrib.lite.Interpreter(joint_model)
    ...
    def detect_hand(self, img_norm):
        ...
        scores = self._sigm(out_clf)
        mask = scores > 0.5
        scores = scores[mask]
        candidate_detect = out_reg[mask]
        candidate_anchors = self.anchors[mask]

        if candidate_detect.shape[0] == 0:
            return []

        center_wo_offst = candidate_anchors[:, :2] * 256
        boxes = np.zeros_like(candidate_detect[:, :4])
        boxes[:, :2] = center_wo_offst - candidate_detect[:, :2] - candidate_detect[:, 2:4]
        boxes[:, 2:4] = center_wo_offst - candidate_detect[:, :2] + candidate_detect[:, 2:4]

        index = nms(boxes, scores, thresh=0.1)
        sources = []
        for i in index:
            dx, dy, w, h = candidate_detect[i, :4]
            center_wo_offst = candidate_anchors[i, :2] * 256

            # 7 initial keypoints
            keypoints = center_wo_offst + candidate_detect[i, 4:].reshape(-1, 2)
    ...
    def __call__(self, img):
        ...
        sources = self.detect_hand(img_norm)
        if len(sources) == 0:
            return [], []
